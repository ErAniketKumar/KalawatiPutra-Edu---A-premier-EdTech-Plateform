...
const MentorshipBookingsManager = ({ bookings, setBookings }) => {
    ...
    useEffect(() => {
        filterBookings();
    }, [bookings, searchTerm, statusFilter, sessionTypeFilter]);
    ...
    const filterBookings = () => {
        ...
        if (sessionTypeFilter !== 'all') {
            filtered = filtered.filter(booking => booking.sessionType === sessionTypeFilter);
        }
    ...
    const updateBookingStatus = async (bookingId, newStatus) => {
        ...
        try {
            const response = await axios.put(
                `${VITE_API_URL}/mentorship-bookings/${bookingId}/status`,
                { status: newStatus },
                {
    ...
    const deleteBooking = async (bookingId) => {
        ...
        try {
            const response = await axios.delete(
                `${VITE_API_URL}/mentorship-bookings/${bookingId}`,
                {
                    headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
                }
            );
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Session Type Stats */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {['one-on-one', 'group', 'workshop', 'career-review'].map(type => {
                    const count = bookings.filter(b => b.sessionType === type).length;
                    return (
                        <div key={type} className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-gray-400 text-sm">{type.replace('-', ' ')}</p>
                                    <p className="text-xl font-bold text-white">{count}</p>
                                </div>
                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getSessionTypeColor(type)}`}>
                                    {type.replace('-', ' ')}
                                </span>
                            </div>
                        </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Search by name, email, mentor, or topic..."
                        value={searchTerm}
                ...
                <div className="relative">
                    ...
                    <select
                        ...
                        className="pl-10 pr-8 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                    >
                                ...
                                <tr key={booking._id} className="hover:bg-gray-700 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="space-y-1">
                                            ...
                                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getSessionTypeColor(booking.sessionType)}`}>
                                                {booking.sessionType.replace('-', ' ')}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`text-sm font-medium capitalize ${getExperienceColor(booking.experience)}`}>
                                            {booking.experience}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-semibold text-white">Mentorship Booking Details</h3>
                                <button
                            ...
                            <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-center">
                                                ...
                                                <span className={`capitalize ${getExperienceColor(selectedBooking.experience)}`}>
                                                    {selectedBooking.experience} Level
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center">
                                                ...
                                                <span className="text-gray-300">{selectedBooking.mentorName}</span>
                                            </div>
                                            <div className="flex items-center">
                                            ...
                                            <div className="flex items-center">
                                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getSessionTypeColor(selectedBooking.sessionType)}`}>
                                ...
                                <div>
                                    <h4 className="text-lg font-medium text-white mb-3">Topic & Goals</h4>
                                    <div className="space-y-4">
                                        <div>
                                            <div className="flex items-center mb-2">
                                                <Target size={16} className="text-blue-400 mr-2" />
                                                <span className="text-sm font-medium text-gray-300">Discussion Topic</span>
                                            </div>
                                            <div className="bg-gray-700 p-3 rounded-lg">
                                                <p className="text-gray-300">{selectedBooking.topic}</p>
                                            </div>
                                        </div>
                                        <div>
                                            <div className="flex items-center mb-2">
                                                <BookOpen size={16} className="text-green-400 mr-2" />
                                                <span className="text-sm font-medium text-gray-300">Goals</span>
                                            </div>
                                            <div className="bg-gray-700 p-3 rounded-lg">
                                                <p className="text-gray-300">{selectedBooking.goals}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {selectedBooking.specialRequests && (
                                    <div>
                                        <div className="flex items-center mb-3">
                                            <MessageCircle size={16} className="text-purple-400 mr-2" />
            ...
            {filteredBookings.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">No mentorship bookings match your current filters.</p>
                </div>
            )}
        </div>
    );
...
export default MentorshipBookingsManager;
